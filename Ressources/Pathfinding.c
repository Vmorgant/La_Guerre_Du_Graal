...
void valeur_elt(t_liste * p, t_noeud * v) {
	if(!hors_liste)
		*v = p->ec -> valeur;
}

void modif_elt(t_liste * p, t_noeud v) {
	if(!hors_liste(p))
		p->ec -> valeur = v;
}
...
void ajout_droit(t_liste * p, t_noeud v) {
...
void ajout_gauche(t_liste * p, t_noeud v) {
	if (liste_vide(p) || !hors_liste(p) ) {
		t_element * nouv;
		nouv = malloc (sizeof(t_element));
		nouv -> valeur = v;
...
int existinf(t_liste liste,t_noeud valeur){
...
int distance(int x,int y,int objx,int objy){
	...
	distance = sqrt((objx-x)*(objx-x)+(objy-y)*(objy-y));
	return distance;
}


void pathfinding(int x, int y,int objx,int objy,t_map map){
